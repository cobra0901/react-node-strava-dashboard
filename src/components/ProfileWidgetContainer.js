import React, { Component } from 'react';

// Import request module
import axios from 'axios';
//import createFragment from 'react-addons-create-fragment'; 

//Import styling
import ProfileWidget from '../components/ProfileWidget';

class ProfileWidgetContainer extends Component {
    constructor() {
        super();

        // Set initial state
        this.state = {
            loading: false,
            values: []
        }

        // Bind function to refer to component
        this.getData = this.getData.bind(this);
    }

    // Fetch data when the component is added
    componentDidMount() {
        this.getData().then(_ => {
            // Re-fetch every minute
            //this.interval = setInterval(this.getData, 60000);
        });
    }

    // Fetch new data
    getData() {
        // Tell the Widget component we're currently loading
        this.setState({ loading: true });

        return axios.get('http://localhost:3001/strava/profile?_id=' + this.props.token)
            .then(response => {
				console.log(response.data)
				//let profileData = createFragment(response.data)
                
				// Update state with data
                this.setState({ loading: false, values: response.data});
            })
            .catch(error => {
                // At least tell the Widget component we have stopped loading
                console.log(error);
                this.setState({ loading: false });
            });
    }

    render() {
        return (
            // Render the list widget
            <ProfileWidget heading={this.props.heading} colspan={this.props.colspan} rowspan={this.props.rowspan} profileDetails={this.state.values} loading={this.state.loading} />
        );
    }
}

// Enforce the type of props to send to this component
ProfileWidgetContainer.propTypes = {
    heading: React.PropTypes.string,
    colspan: React.PropTypes.number,
    rowspan: React.PropTypes.number,
    href: React.PropTypes.string.isRequired
}

export default ProfileWidgetContainer;